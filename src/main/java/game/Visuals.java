...
public class Visuals {
    ...
    public void drawGame(Graphics g, Rectangle player) {
        g.drawImage(gameBackground, 0, 0, Constants.WINDOWSIZE, Constants.MAPMAXHEIGHT, null);
        g.drawImage(gameGround, 0, Constants.MAPMAXHEIGHT, Constants.WINDOWSIZE, Constants.WINDOWSIZE - Constants.MAPMAXHEIGHT, null);
    ...
    public void gameOverScreen(Graphics g, int score) {
        ...
        g.drawString("Game Over!", 300, Constants.WINDOWSIZE / 2 - 150);
        if (score <= 10) tooLong = 0;
        else tooLong = 5;
        g.drawString("Your score: " + score, 270 - tooLong, Constants.WINDOWSIZE / 2 - 50);
        g.setFont(minecraftNormalFont);
        g.drawString("Click to play again", 240, Constants.WINDOWSIZE / 2 + 50);
        g.drawImage(homeButton, 0, 0, 75, 65, null);
    }

    public void homeScreen(Graphics g) {
        g.drawImage(gameMenu, 0, 0, Constants.WINDOWSIZE, Constants.WINDOWSIZE, null);

        g.setColor(Color.GRAY);
        g.setFont(minecraftTitleFont);

        g.drawString("Flappy Creep!", 100, Constants.WINDOWSIZE / 2 - 150);
        ...
        g.drawString("FlappyCreep v1.0.0", 20, Constants.WINDOWSIZE - 50);
    ...
    public void startBanner(Graphics g) {
        g.setFont(minecraftTitleFont);
        g.drawString("Click to start!", 75, Constants.WINDOWSIZE / 2 - 50);
    }
